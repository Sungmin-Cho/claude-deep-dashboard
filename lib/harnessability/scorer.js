 ...
 *
 * `saveReport()` wraps the score result in the deep-suite M3 cross-plugin
 * envelope (cf. deep-suite/docs/envelope-migration.md §1, §4) so consumers
...
// ---------------------------------------------------------------------------
//
// Mirror of deep-suite/scripts/wrap-artifact.js generateUlid. Implementation
...
function detectGit(projectRoot) {
  ...
  if (!head || !/^[a-f0-9]{7,40}$/.test(head)) {
    return { head: '0000000', branch: 'HEAD', dirty: 'unknown' };
  }
  const branch = runGit(['rev-parse', '--abbrev-ref', 'HEAD'])?.trim() || 'HEAD';
  const status = runGit(['status', '--porcelain']);
  return { head, branch, dirty: status === null ? 'unknown' : status.length > 0 };
}
...
// CLI entry
// ---------------------------------------------------------------------------
// When invoked as `node scorer.js --project-root <projectRoot>` (or with the
// single positional compatibility form), compute the score for that project,
// save the envelope-wrapped report, and emit the envelope JSON on stdout.
// The target root is always explicit; this entrypoint never falls back to cwd.
// This entry block is a no-op when scorer.js is imported as a module.

function scorerUsageError(message) {
  const error = new Error(message);
  error.code = 'USAGE';
  return error;
}

function parseScorerProjectRoot(argv) {
  let flaggedRoot = null;
  let positionalRoot = null;

  for (let index = 0; index < argv.length; index += 1) {
    const arg = argv[index];
    if (arg === '--project-root') {
      if (flaggedRoot !== null) throw scorerUsageError('--project-root may be supplied only once');
      const value = argv[index + 1];
      if (value === undefined || value === '' || value.startsWith('--')) {
        throw scorerUsageError('--project-root requires a path');
      }
      flaggedRoot = value;
      index += 1;
      continue;
    }
    if (arg.startsWith('--')) throw scorerUsageError(`unknown argument: ${arg}`);
    if (arg === '') throw scorerUsageError('project root must not be empty');
    if (positionalRoot !== null) throw scorerUsageError('only one positional project root is allowed');
    positionalRoot = arg;
  }

  if (flaggedRoot !== null && positionalRoot !== null) {
    throw scorerUsageError('do not combine a positional root with --project-root');
  }
  const projectRoot = flaggedRoot ?? positionalRoot;
  if (projectRoot === null) throw scorerUsageError('an explicit project root is required');
  return path.resolve(projectRoot);
}

if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  try {
    const projectRoot = parseScorerProjectRoot(process.argv.slice(2));
    const result = await scoreHarnessability(projectRoot);
    const { envelope } = saveReport(projectRoot, result);
    console.log(JSON.stringify(envelope, null, 2));
  } catch (error) {
    if (error?.code === 'USAGE') {
      console.error(`harnessability-scorer: ${error.message}`);
      console.error('Usage: node scorer.js [--project-root PATH | PATH]');
      process.exitCode = 2;
    } else {
      console.error(`harnessability-scorer: ${error instanceof Error ? error.message : String(error)}`);
      process.exitCode = 1;
    }
  }
}
