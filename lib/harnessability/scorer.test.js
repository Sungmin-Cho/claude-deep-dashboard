...
test('scorer.js CLI writes envelope JSON to stdout and saves envelope-wrapped harnessability-report.json', () => {
  ...
  const result = spawnSync(process.execPath, [scorerPath, root], { encoding: 'utf8' });

  assert.equal(result.status, 0, `scorer CLI should exit 0; got ${result.status}. stderr: ${result.stderr}`);

  // stdout is the M3 envelope object (cf. deep-suite/docs/envelope-migration.md §1).
  let parsed;
  try {
...
test('scorer.js CLI requires one explicit root and accepts --project-root', () => {
  const root = mktemp();
  const externalCwd = mktemp();
  writeFile(root, 'main.py', 'print("hello")');

  const scorerPath = cliFileURLToPath(new URL('./scorer.js', import.meta.url));
  const missing = spawnSync(process.execPath, [scorerPath], { cwd: externalCwd, encoding: 'utf8' });
  assert.equal(missing.status, 2, missing.stderr);

  const explicit = spawnSync(
    process.execPath,
    [scorerPath, '--project-root', root],
    { cwd: externalCwd, encoding: 'utf8' }
  );
  assert.equal(explicit.status, 0, explicit.stderr);
  assert.equal(JSON.parse(explicit.stdout).payload.projectRoot, path.resolve(root));

  const duplicate = spawnSync(
    process.execPath,
    [scorerPath, '--project-root', root, '--project-root', root],
    { cwd: externalCwd, encoding: 'utf8' }
  );
  assert.equal(duplicate.status, 2, duplicate.stderr);

  // An empty positional root must be rejected, not resolved to process.cwd().
  const emptyPositional = spawnSync(process.execPath, [scorerPath, ''], {
    cwd: externalCwd,
    encoding: 'utf8',
  });
  assert.equal(emptyPositional.status, 2, emptyPositional.stderr);
