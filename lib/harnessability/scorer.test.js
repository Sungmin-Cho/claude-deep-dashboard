...
test('scorer.js CLI writes JSON to stdout and saves harnessability-report.json', () => {
  const root = mktemp();
  writeFile(root, 'main.py', 'print("hello")');
  ...
  // stdout should be parseable JSON with expected shape
  let parsed;
  try {
  ...
  assert.equal(typeof parsed.total, 'number', 'result.total must be a number');
  assert.ok(Array.isArray(parsed.dimensions), 'result.dimensions must be an array');
  assert.equal(parsed.dimensions.length, 6, 'should have 6 dimensions');

  // And the report file must exist
  ...
  assert.equal(reportContent.total, parsed.total, 'report file should match stdout JSON');
});
