...
import { AGGREGATOR_KINDS, EXPECTED_SOURCES, KNOWN_SUITE_PLUGINS } from './suite-constants.js';
...
// deep-suite docs/test-catalog.md §1-§8). The deferred map is now
...
// lockstep with deep-suite docs/test-catalog.md (similar to the
...
/**
 * DEPRECATED metric stub — suite.wiki.auto_ingest_candidates_total.
 *
 * The catalog definition ("SessionStart auto-ingest candidate detection
 * count from deep-wiki log.jsonl") is unimplementable: deep-wiki's
 * log.jsonl has no candidate-detection action — detection only leaves a
 * transient `.wiki-meta/.pending-scan` watermark (consumed, not durable).
 * The previous implementation matched the fictional vocabulary
 * `event: 'auto-ingest-candidate' | 'session-start-detect'` and was
 * therefore always 0 in real data.
 *
 * Wire-key contract: the key stays in every snapshot (removing it silently
 * breaks formatters / mirror tests enumerating snapshot keys — key removal
 * is a versioned contract change only) with value pinned to null
 * ("unmeasurable = null" semantics). Do NOT route ingest throughput into
 * this name — manual/batch ingests are not "auto-ingest candidates".
 */
function deprecatedAutoIngestCandidatesTotal() {
  return {
    value: null,
    source_summary: {
      deprecated: true,
      reason: 'no durable candidate signal in producer (deep-wiki)',
      replaced_by: 'suite.wiki.ingest_actions_total',
    },
  };
}

// deep-wiki log.jsonl real vocabulary (wiki-schema.yaml): lines are
// {ts, action, pages_created, pages_updated, source} — the field is
// `action`, and ingest-lifecycle values are the 4 below. `ingest-fail`
// counts too: failures are activity, and excluding them would disguise a
// failure storm as reduced activity.
const WIKI_INGEST_ACTIONS = new Set(['ingest', 'ingest-skip', 'ingest-repair', 'ingest-fail']);

function computeWikiIngestActionsTotal(ndjsonLogs) {
  ...
  if (wikiLog.error) return { value: null, source_summary: { error: wikiLog.error.reason || true } };
  let total = 0;
  for (const e of wikiLog.events) {
    if (e && WIKI_INGEST_ACTIONS.has(e.action)) {
      total += 1;
    }
  }
...
// ---------------------------------------------------------------------------
//
// Source: deep-suite/schemas/{handoff,compaction-state}.schema.json
...
// M5.5 standard test catalog (deep-suite docs/test-catalog.md §1-§8).
//
...
// KNOWN_SUITE_PLUGINS imported from ./suite-constants.js (hoisted in v1.3.3 — W2).
...
export function buildSnapshot(collected, options = {}) {
  ...
  // M4-core (13 — includes the deprecated auto_ingest_candidates_total wire
  // key, pinned null, and its honest replacement ingest_actions_total)
  ...
  wrap('suite.wiki.auto_ingest_candidates_total','M4-core', 'count',    deprecatedAutoIngestCandidatesTotal());
  wrap('suite.wiki.ingest_actions_total',        'M4-core', 'count',    computeWikiIngestActionsTotal(ndjson_logs));
  ...
  // Source: lib/test-catalog-manifest.json (mirrors deep-suite
...
export const _internal = {
  ...
  deprecatedAutoIngestCandidatesTotal,
  computeWikiIngestActionsTotal,
