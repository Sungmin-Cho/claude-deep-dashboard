...
// M5 activation (2026-05-11): the 3 M5-gated metrics promoted to M4-core; the
// M5.5-gated metric (per-plugin test coverage from CI aggregation) stays
// deferred. See claude-deep-suite PRs #11/#12/#13 for the schema ratification
// that unblocks the activation.
...
function computeCompactionFrequency(envelopeSources) {
  const src = envelopeSources.find(
    (s) => s.producer === 'deep-work' && s.kind === 'compaction-state'
  );
  if (!src || src.envelopes.length === 0) {
  ...
  return {
    value: src.envelopes.length,
    source_summary: {
      total_events: src.envelopes.length,
...
function computeHandoffRoundtripSuccessRate(envelopeSources) {
  const handoffSrc = envelopeSources.find(
    (s) => s.producer === 'deep-work' && s.kind === 'handoff'
  );
  if (!handoffSrc || handoffSrc.envelopes.length === 0) {
    return { value: null, source_summary: { handoffs_total: 0 } };
  }
  // Build parent_run_id index across ALL non-handoff envelope sources + the
  // handoff source itself (a reverse handoff is a handoff that chains back).
  // Per long-run-handoff.md §7: roundtrip = receiver emits ANY envelope whose
  // parent_run_id points at the original handoff's run_id.
  ...
  for (const src of envelopeSources) {
    for (const env of src.envelopes) {
      ...
      if (typeof parent === 'string' && parent.length > 0 && parent !== self) {
        childrenByParent.add(parent);
      }
    }
  }
  let roundtripped = 0;
  for (const env of handoffSrc.envelopes) {
    const rid = env.envelope?.run_id;
    if (typeof rid === 'string' && rid.length > 0 && childrenByParent.has(rid)) {
      roundtripped += 1;
    }
  }
  return {
    value: roundtripped / handoffSrc.envelopes.length,
    source_summary: {
      handoffs_total: handoffSrc.envelopes.length,
...
export function buildSnapshot(collected, options = {}) {
  ...
  wrap('suite.dashboard.missing_signal_ratio',   'M4-core', 'ratio',
       { value: collected.missing_signal_ratio,
         source_summary: { expected_total: 13, denominator_includes: 'envelope+ndjson' } });
  ...
  // M4-deferred (1 — suite.tests.coverage_per_plugin, M5.5)
