...
function computeCompactionFrequency(envelopeSources) {
  ...
  return {
    ...
    source_summary: {
      ...
      compaction_producers: sources.map((s) => s.producer).sort(),
    },
  };
}
...
function computeHandoffRoundtripSuccessRate(envelopeSources) {
  ...
  // (reverse). Per long-run-handoff.md §7, both directions count as handoffs
  // that can round-trip.
  ...
  // child iteration. Catalog contract (metrics-catalog.yaml + this function's
  // doc comment) specifies "downstream non-aggregator envelope's parent_run_id
  // chains back" — aggregator artifacts (harnessability-report, evolve-insights,
  // index) by design don't continue a thread.
  //
  // Per long-run-handoff.md §7: roundtrip = receiver emits ANY non-aggregator
  // envelope whose parent_run_id points at the original handoff's run_id.
  ...
  return {
    value: roundtripped / allHandoffs.length,
    source_summary: {
      handoffs_total: allHandoffs.length,
      handoffs_roundtripped: roundtripped,
      handoff_producers: handoffSources.map((s) => s.producer).sort(),
    },
  };
}
