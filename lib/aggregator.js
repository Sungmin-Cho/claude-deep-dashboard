 ...
 *      `^\s*<TOKEN>\b` against the verdict-line tail. This avoids substring
 *      poisoning from prose like "APPROVE — no CONCERN raised" (Round 1
 *      review: Opus W1).
 *   2. FALLBACK — if no leading token, scan the verdict line for any token
 *      in severity order (REQUEST_CHANGES > CONCERN > APPROVE). Used when the
 *      verdict line is itself a table cell or quoted text where leading-token
 *      anchoring fails.
 *   3. LAST RESORT — if no `**Verdict**:` line, do a severity-ordered scan
...
function parseVerdictFromMarkdown(text) {
  ...
  if (verdictLineMatch) {
    ...
    // Strip markdown emphasis (`**APPROVE**` / `*APPROVE*`) and surrounding
    // backticks at the start, then anchor on a word-boundaried verdict token.
    const cleaned = tail.replace(/^[\s*`_]+/, '');
    for (const t of VERDICT_TOKENS) {
      const re = new RegExp(`^${t}\\b`);
      if (re.test(cleaned)) return t;
    }
    // Fallback: severity-ordered substring scan inside the verdict line only.
    // (Catches table-cell verdicts like "| ✅ | APPROVE |".)
    for (const t of VERDICT_TOKENS) {
      const re = new RegExp(`\\b${t}\\b`);
      if (re.test(tail)) return t;
    }
    return null;
  }
  // No **Verdict**: line — severity-ordered whole-document scan.
  for (const t of VERDICT_TOKENS) {
    const re = new RegExp(`\\b${t}\\b`);
