...
test('handoff_roundtrip_success_rate: reverse handoff (deep-evolve producer) also counts (Round 1: C1)', () => {
  ...
  // Round 2 W2 fix: tighten payload.to.producer for H1 to match the H2
  // envelope's producer (deep-evolve). The reverse handoff H2 itself targets
  // deep-work but no envelope chains back to H2 → H2 not roundtripped.
  envelopeSources[0].envelopes[0].payload = {
    handoff_kind: 'phase-5-to-evolve',
    to: { producer: 'deep-evolve' },
  };
  envelopeSources[1].envelopes[0].payload = {
    handoff_kind: 'evolve-to-deep-work',
    to: { producer: 'deep-work' },
  };
  const r = _internal.computeHandoffRoundtripSuccessRate(envelopeSources);
  // Both H1 and H2 are handoffs. H1's expected receiver is deep-evolve;
  // H2's producer = deep-evolve and parent_run_id = H1 → H1 roundtripped.
  // H2's expected receiver is deep-work; no envelope chains back to H2 → H2 not.
  assert.equal(r.source_summary.handoffs_total, 2);
  assert.equal(r.source_summary.handoffs_roundtripped, 1);
  // Round 2 W3 fix: only producers with non-empty envelopes appear.
