...
// ---------------------------------------------------------------------------
// Test: snapshot shape — all 17 metrics emitted, deferred carry deferred_until
// ---------------------------------------------------------------------------

test('buildSnapshot emits all 17 metrics with required envelope (M5 + M5.5 activation)', async () => {
  const root = mktemp();
  ...
  assert.equal(metricIds.length, 17);

  // After M5.5 activation (2026-05-12): all metrics are M4-core. The
  ...
  assert.equal(coreIds.length, 17);
...
test('greenfield: M4-core metrics that depend on absent sources emit null', async () => {
  ...
  const expectedNullCore = [
    ...
    'suite.wiki.ingest_actions_total',
...
test('block_rate ignores non-hook NDJSON sources (Round 1: 3-way HIGH)', () => {
  ...
  const ndjsonLogs = [
    ...
    // Wiki vault log — many events, NONE should enter denominator
    {
      producer: 'deep-wiki',
      kind: 'log',
      events: Array.from({ length: 200 }, () => ({ ts: '2026-07-01T10:00:00Z', action: 'ingest', pages_created: [], pages_updated: [], source: 's.md' })),
      missing: false,
    },
  ];
...
// ---------------------------------------------------------------------------
// wiki metrics — real deep-wiki log.jsonl vocabulary
// ---------------------------------------------------------------------------
//
// deep-wiki log.jsonl lines are {ts, action, pages_created, pages_updated,
// source} (wiki-schema.yaml) — the field is `action`, and the lifecycle
// vocabulary has no 'auto-ingest-candidate'/'session-start-detect' values.
// The old auto_ingest_candidates_total metric consumed that fictional
// vocabulary and was therefore always 0 in real data → deprecated (wire key
// preserved, value pinned to null). ingest activity is now measured honestly
// by ingest_actions_total.

test('wiki ingest_actions_total counts the 4 ingest lifecycle actions from real log lines', () => {
  const line = (action) => ({
    ts: '2026-07-01T10:00:00Z',
    action,
    pages_created: [],
    pages_updated: [],
    source: 'notes/inbox.md',
  });
  const ndjsonLogs = [
    {
      producer: 'deep-wiki',
      ...
      events: [
        ...
        line('ingest-skip'),
        line('ingest-repair'),
        line('ingest-fail'), // failures ARE activity — excluding them would disguise failure storms as quiet
        line('update'),
        line('lint'),
        line('rebuild'),
        line('delete'),
  ...
  const r = _internal.computeWikiIngestActionsTotal(ndjsonLogs);
  assert.equal(r.value, 4);
  assert.equal(r.source_summary.matched, 4);
});

test('wiki ingest_actions_total is null when the wiki log source is missing', () => {
  const r = _internal.computeWikiIngestActionsTotal([
    { producer: 'deep-wiki', kind: 'log', events: [], missing: true },
  ]);
  assert.equal(r.value, null);
});

test('wiki ingest_actions_total is null when the wiki log read failed, even with partial events', () => {
  // readNdjson (suite-collector.js) returns partial `events` plus an `error`
  // field on stream/IO failure. Counting the partial events would leak a
  // read failure as a concrete (and misleadingly low) activity count,
  // violating metrics-catalog.yaml's `null_when: "log.jsonl missing or
  // unreadable"` contract.
  const line = (action) => ({
    ts: '2026-07-01T10:00:00Z',
    action,
  ...
  const r = _internal.computeWikiIngestActionsTotal([
    {
      producer: 'deep-wiki',
      kind: 'log',
      events: [line('ingest'), line('ingest-fail')],
      missing: false,
      error: { reason: 'stream-error', message: 'boom' },
    },
  ]);
  assert.equal(r.value, null);
  assert.equal(r.source_summary.error, 'stream-error');
});

test('deprecated auto_ingest_candidates_total: wire key MUST exist in snapshot with value pinned to null', async () => {
  // Wire-key preservation contract: removing the key (or emitting non-null)
  // is a breaking contract change — formatters/mirror tests enumerate
  // snapshot keys. Key absence must fail this test.
  const root = mktemp();
  writeText(
    root,
    '.deep-wiki/log.jsonl',
    [
      JSON.stringify({ ts: '2026-07-01T10:00:00Z', action: 'ingest', pages_created: ['p'], pages_updated: [], source: 's.md' }),
      JSON.stringify({ ts: '2026-07-01T11:00:00Z', action: 'ingest-fail', pages_created: [], pages_updated: [], source: 's.md' }),
    ].join('\n') + '\n'
  );
  const collected = await collectSuite(root);
  const snap = buildSnapshot(collected, { nowIso: '2026-07-01T12:00:00Z' });

  assert.ok(
    Object.prototype.hasOwnProperty.call(snap.metrics, 'suite.wiki.auto_ingest_candidates_total'),
    'deprecated wire key must be preserved'
  );
  assert.equal(snap.metrics['suite.wiki.auto_ingest_candidates_total'].value, null);

  // ...and the honest metric counts the same log's ingest activity.
  assert.equal(snap.metrics['suite.wiki.ingest_actions_total'].value, 2);
});

test('metrics-catalog.yaml stays in sync with the wiki metric deprecation', () => {
  // Zero-dep repo (no YAML parser): anchor on the literal catalog text. If
  // the catalog entry is renamed/removed without updating the aggregator
  // (or vice versa), these anchors surface the drift.
  const catalogPath = path.join(path.dirname(fileURLToPath(import.meta.url)), 'metrics-catalog.yaml');
  const catalog = fs.readFileSync(catalogPath, 'utf8');

  const candidatesEntry = catalog.split('- id: suite.wiki.auto_ingest_candidates_total')[1]?.split('\n  - id:')[0] ?? '';
  assert.ok(candidatesEntry.includes('deprecated: true'), 'deprecated flag must be in the catalog entry');
  assert.ok(candidatesEntry.includes('replaced_by: suite.wiki.ingest_actions_total'));

  assert.ok(catalog.includes('- id: suite.wiki.ingest_actions_total'), 'new metric must be catalogued');
  assert.ok(catalog.includes("'ingest-fail'"), 'failure actions must be part of the documented aggregation');
});
...
test('appendSnapshot creates JSONL file in .deep-dashboard/', async () => {
  ...
  assert.ok(out.endsWith(path.join('.deep-dashboard', 'suite-metrics.jsonl')));
...
test('compaction_preserved_artifact_ratio: undefined when discarded_artifact_paths omitted (guide §5)', () => {
  // Per deep-suite/guides/context-management.md §5: when
...
test('end-to-end: M5 fixtures populate compaction + handoff metrics (no roundtrip)', async () => {
  ...
  // the M5 envelope shapes — mirror of deep-suite schemas).
...
// deep-suite docs/test-catalog.md §1-§8 cross-reference).
