...
test('handoff_roundtrip_success_rate: reverse handoff also counts as roundtrip', () => {
  // long-run-handoff.md §7: a reverse `evolve-to-deep-work` handoff chained to
  // the original handoff is also a roundtrip signal.
  const envelopeSources = [
    {
      producer: 'deep-work',
      kind: 'handoff',
      envelopes: [
        ...
        // Reverse handoff — H2 chains back to H1. H1 round-tripped.
        {
...
test('end-to-end: M5 fixtures populate compaction + handoff metrics', async () => {
  const root = mktemp();
