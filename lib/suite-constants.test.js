...
test('EXPECTED_SOURCES has 11 entries (8 envelope + 3 ndjson) after Round 1 scope expansion', () => {
  // Pre-Round-1 was 8 (envelope-only). Round 1 (Codex adv HIGH) expanded the
  // denominator to include observable NDJSON sources: deep-work/hook-log,
  // deep-evolve/hook-log, deep-wiki/log. The deep-review review-report
  // markdown source remains deferred to PR 2's verdict_mix formatter.
  assert.equal(EXPECTED_SOURCES.length, 11);
  ...
  assert.equal(envelopeEntries.length, 8);
...
test('PAYLOAD_REQUIRED_FIELDS covers all 8 envelope-typed sources', () => {
