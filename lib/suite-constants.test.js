...
test('PAYLOAD_REQUIRED_FIELDS for handoff mirrors schemas/handoff.schema.json required[]', () => {
  // Source: deep-suite/schemas/handoff.schema.json (M5, schema_version 1.0).
...
test('PAYLOAD_REQUIRED_FIELDS for compaction-state mirrors schemas/compaction-state.schema.json required[]', () => {
  // Source: deep-suite/schemas/compaction-state.schema.json (M5, schema_version 1.0).
...
test('KNOWN_SUITE_PLUGINS exports the 7 known suite slugs (suite + 6 plugins) frozen (W2)', () => {
  assert.ok(Array.isArray(KNOWN_SUITE_PLUGINS), 'array export');
  assert.deepEqual(
    [...KNOWN_SUITE_PLUGINS].sort(),
    ['deep-dashboard', 'deep-docs', 'deep-evolve', 'deep-review', 'deep-wiki', 'deep-work', 'suite']
  );
  assert.ok(Object.isFrozen(KNOWN_SUITE_PLUGINS), 'must be frozen');
});
