...
test('M5 C2: broken symlink at per-session handoff path is silently skipped', SYMLINK_TEST_OPTIONS, async () => {
  ...
  // Broken symlink rejection happens at the `pathExists` short-circuit
  // (`fs.accessSync` follows symlinks → ENOENT for missing target → false).
  // The downstream realpath containment check is not reached for broken
  // symlinks. Treated like a non-existent file at the canonical path:
  // silently skipped, no failure recorded (matches the "no producer yet"
  // case so missing_signal_ratio remains the single source of "this path
  // produced nothing").
  assert.equal(handoffs.envelopes.length, 0);
  // Round 3 I10 polish: assert no failure recorded — confirms the path-exists
  // short-circuit, not the realpath check, is the rejection mechanism.
