...
test('greenfield project → missing_signal_ratio = 1.0, chains.completeness = null', async () => {
  ...
  assert.equal(result.missing_signal_ratio, 1.0); // all 13 expected sources missing
...
test('missing_signal_ratio reflects 13-source denominator (envelopes + NDJSON logs)', async () => {
  const root = mktemp();
  // Provide only 1 of 13 expected sources (the session-receipt envelope).
  writeJson(
    root,
    '.deep-work/session-receipt.json',
    envelope({
  ...
  // 12 of 13 expected sources missing.
  const expected = 12 / EXPECTED_SOURCES.length;
  assert.ok(
...
test('missing_signal_ratio counts an empty hook log as missing (Round 1: Codex adv HIGH)', async () => {
  ...
  const r = await collectSuite(root);
  // All 13 expected sources missing (empty file = missing signal).
...
test('missing_signal_ratio counts NDJSON stream-error as missing', async () => {
  ...
  const r = await collectSuite(root);
  // 1 of 13 present → 12/13 missing (~0.923).
