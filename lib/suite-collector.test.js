...
const SYMLINK_TEST_OPTIONS = probeSymlinkCapability();
...
const VALID = {
  ...
  // M5: handoff (deep-suite/schemas/handoff.schema.json required[])
  'deep-work/handoff': {
    schema_version: '1.0',
  ...
  // M5: compaction-state (deep-suite/schemas/compaction-state.schema.json required[])
...
// ---------------------------------------------------------------------------
// Test: payload schema.version MAJOR guard (suite contract: MAJOR mismatch →
// skip + warning; MINOR is additive and must unwrap)
// ---------------------------------------------------------------------------

test('payload schema MAJOR bump (v2.0) is rejected with schema-major-mismatch and counted', async () => {
  // Suite contract (schemas/README.md, envelope-migration.md): payload
  // schema.version evolves independently per producer; a breaking MAJOR bump
  // means v1.0 field assumptions no longer hold. Unwrapping it would let
  // aggregators silently compute wrong values (e.g. renamed fields → NaN → 0).
  const root = mktemp();
  const v2 = envelope({
    producer: 'deep-work',
    kind: 'session-receipt',
  ...
  v2.envelope.schema.version = '2.0';
  writeJson(root, '.deep-work/session-receipt.json', v2);
  ...
  assert.equal(session.envelopes.length, 0);
  assert.equal(session.failures[0].reason, 'schema-major-mismatch:2.0');
  assert.equal(result.schema_failures_total, 1);
});

test('payload schema MINOR bump (v1.1, additive) unwraps normally', async () => {
  // MINOR bumps are additive by suite contract — blocking them would make a
  // normal producer upgrade look like an observability gap.
  const root = mktemp();
  const v11 = envelope({
    producer: 'deep-work',
    kind: 'session-receipt',
  ...
  v11.envelope.schema.version = '1.1';
  writeJson(root, '.deep-work/session-receipt.json', v11);
...
test('non-numeric payload schema.version is defensively rejected as MAJOR mismatch', async () => {
  const root = mktemp();
  const bad = envelope({
    producer: 'deep-work',
    kind: 'session-receipt',
  ...
  bad.envelope.schema.version = 'latest';
  writeJson(root, '.deep-work/session-receipt.json', bad);
  ...
  assert.equal(session.envelopes.length, 0);
  assert.equal(session.failures[0].reason, 'schema-major-mismatch:latest');
  assert.equal(result.schema_failures_total, 1);
});
...
test('project-local wiki index fallback includes the .wiki-meta segment', () => {
  // wiki-schema.yaml: index.json lives at <wiki_root>/.wiki-meta/index.json.
  // The explicit/env branches already resolve correctly; the project-local
  // fallback dropped the .wiki-meta segment and pointed at a path that never
  // exists in a real vault.
  const prevRoot = process.env.DEEP_WIKI_ROOT;
  const prevVault = process.env.DEEP_WIKI_VAULT_ROOT;
  delete process.env.DEEP_WIKI_ROOT;
  delete process.env.DEEP_WIKI_VAULT_ROOT;
  try {
    const resolved = _internal.resolveWikiIndexPath('/proj');
    assert.equal(resolved, path.join('/proj', '.deep-wiki', '.wiki-meta', 'index.json'));
  } finally {
    if (prevRoot !== undefined) process.env.DEEP_WIKI_ROOT = prevRoot;
    if (prevVault !== undefined) process.env.DEEP_WIKI_VAULT_ROOT = prevVault;
  }
});
...
test('M5 C1: handoff envelopes at per-session subdir (.deep-work/<session>/handoff.json) are discovered', async () => {
  // deep-suite/guides/long-run-handoff.md §4.1 shows the canonical emit
...
test('M5 C2: out-of-tree symlink at per-session handoff path is rejected', SYMLINK_TEST_OPTIONS, async () => {
...
test('M5 C2: broken symlink at per-session handoff path is silently skipped', SYMLINK_TEST_OPTIONS, async () => {
  const root = mktemp();
  const candidate = path.join(root, '.deep-work/2026-05-11-broken/handoff.json');
  fs.mkdirSync(path.join(root, '.deep-work/2026-05-11-broken'), { recursive: true });
  fs.symlinkSync(
    path.join(root, '.deep-work/missing-target/handoff.json'),
    candidate,
    'file'
  );

  // `candidate` is a symlink to an in-tree target that was never emitted. The
  // fix probes it with lstat (which sees the symlink directory entry, no
  // ENOENT), then statSync follows the link and raises ENOENT for the missing
  // target — so the dangling case is skipped at the stat stage
  // (suite-collector.js "stat follows the link" branch) without recording a
  // failure. This deliberately does NOT depend on accessSync, whose
  // OS-dependent dangling-symlink behavior caused the original Windows
  // regression this fix removes.
  const r = await collectSuite(root);
  const handoffs = r.sources.envelopes.find(
    (s) => s.producer === 'deep-work' && s.kind === 'handoff'
  );
  // A dangling producer path is equivalent to a not-yet-emitted artifact:
  // silently skipped so missing_signal_ratio remains the single signal that
  // this source produced nothing.
  assert.equal(handoffs.envelopes.length, 0);
  assert.equal(handoffs.failures.length, 0);
});

test('M5 C2: symlink target vanishing between stat and realpath is silently skipped', SYMLINK_TEST_OPTIONS, (t) => {
  // Deterministic POSIX reproduction of the stat -> realpath TOCTOU race the
  // fix guards against: the symlink target exists at stat time but is gone by
  // realpath. The realpath-stage `candidateIsSymlink && ENOENT` branch must
  // preserve dangling-link semantics (silent skip, no failure) rather than
  // fail closed. Without that branch this would record `candidate-realpath-failed`.
  const root = mktemp();
  const target = path.join(root, '.deep-work/race-target.json');
  const candidate = path.join(root, '.deep-work/race/handoff.json');
  writeJson(root, '.deep-work/race-target.json', { probe: 'race' });
  fs.mkdirSync(path.dirname(candidate), { recursive: true });
  fs.symlinkSync(target, candidate, 'file');

  const realpathSync = fs.realpathSync;
  t.mock.method(fs, 'realpathSync', (absPath, ...args) => {
    // Only the symlink candidate races away; the parent realpath (resolved
    // first) and every other path resolve normally.
    if (path.resolve(absPath) === path.resolve(candidate)) {
      throw Object.assign(new Error('injected realpath ENOENT'), { code: 'ENOENT' });
    }
    return Reflect.apply(realpathSync, fs, [absPath, ...args]);
  });

  const result = _internal.readSessionGlob(root, '.deep-work', 'handoff.json', 'handoffs');

  assert.equal(result.entries.length, 0);
  assert.equal(result.failures.length, 0);
});

test('M5 C2: unexpected lstat/stat/realpath errors fail closed', SYMLINK_TEST_OPTIONS, (t) => {
  const root = mktemp();
  const lstatCandidate = path.join(root, '.deep-work/lstat-error/handoff.json');
  const statCandidate = path.join(root, '.deep-work/stat-error/handoff.json');
  const realpathCandidate = path.join(root, '.deep-work/realpath-error/handoff.json');
  const statTarget = path.join(root, '.deep-work/stat-target.json');

  writeJson(root, '.deep-work/lstat-error/handoff.json', { probe: 'lstat' });
  writeJson(root, '.deep-work/stat-target.json', { probe: 'stat' });
  fs.mkdirSync(path.dirname(statCandidate), { recursive: true });
  fs.symlinkSync(statTarget, statCandidate, 'file');
  writeJson(root, '.deep-work/realpath-error/handoff.json', { probe: 'realpath' });

  const lstatSync = fs.lstatSync;
  const statSync = fs.statSync;
  const realpathSync = fs.realpathSync;
  t.mock.method(fs, 'lstatSync', (absPath, ...args) => {
    if (path.resolve(absPath) === path.resolve(lstatCandidate)) {
      throw Object.assign(new Error('injected lstat failure'), { code: 'EIO' });
    }
    return Reflect.apply(lstatSync, fs, [absPath, ...args]);
  });
  t.mock.method(fs, 'statSync', (absPath, ...args) => {
    if (path.resolve(absPath) === path.resolve(statCandidate)) {
      throw Object.assign(new Error('injected stat failure'), { code: 'EACCES' });
    }
    return Reflect.apply(statSync, fs, [absPath, ...args]);
  });
  t.mock.method(fs, 'realpathSync', (absPath, ...args) => {
    if (path.resolve(absPath) === path.resolve(realpathCandidate)) {
  ...
  assert.deepEqual(
    new Map(result.failures.map(({ path: failurePath, reason }) => [failurePath, reason])),
    new Map([
      [lstatCandidate, 'candidate-lstat-failed'],
      [statCandidate, 'symlink-target-stat-failed'],
      [realpathCandidate, 'candidate-realpath-failed'],
    ])
  );
});

test('M5 W4: dir+session-glob dedup by run_id (Round 3: Codex P2)', async () => {
  // Codex round-3 review P2: a producer writing the same envelope to BOTH
  // layouts (flat + per-session) would otherwise inflate compaction.frequency
  // and handoff.roundtrip denominator. Dedup is by envelope.run_id within
  // the merged source result; the duplicate is logged as `duplicate-run-id`
  // failure for producer-side debugging.
  const root = mktemp();
  const handoffEnv = envelope({
    producer: 'deep-work',
    kind: 'handoff',
    run_id: 'shared-rid-X1',
    payload: VALID['deep-work/handoff'],
  });
  // Write the SAME envelope to both layouts.
  writeJson(root, '.deep-work/handoffs/h-001.json', handoffEnv);
  writeJson(root, '.deep-work/2026-05-11-sess/handoff.json', handoffEnv);

  const r = await collectSuite(root);
  const handoffs = r.sources.envelopes.find(
    (s) => s.producer === 'deep-work' && s.kind === 'handoff'
  );
  // Dedup keeps the first instance (flat dir scanned first) — exactly 1
  // envelope ingested.
  assert.equal(handoffs.envelopes.length, 1);
  // The second instance is recorded as a duplicate-run-id failure for
  // producer-side debugging.
  assert.ok(
    handoffs.failures.some((f) => f.reason === 'duplicate-run-id'),
    `expected duplicate-run-id failure, got: ${JSON.stringify(handoffs.failures)}`
  );
});

test('M5 C2 (I9 expansion): symlinked SUBDIR pointing outside is rejected (Round 3 polish)', SYMLINK_TEST_OPTIONS, async () => {
  // Sibling to the round-2 C2 test (which symlinks the FILE). This one
  // symlinks the entire session SUBDIR to an out-of-tree location. The
  // parent-realpath + candidate-realpath check catches both vectors.
  const root = mktemp();
  const outsideDir = mktemp('outside-subdir-');
  // Create the forged envelope inside the out-of-tree subdir
  fs.mkdirSync(path.join(outsideDir, 'fake-session'), { recursive: true });
  writeJson(
    outsideDir,
    'fake-session/handoff.json',
    envelope({
      producer: 'deep-work',
      kind: 'handoff',
      run_id: 'subdir-hijack',
      payload: VALID['deep-work/handoff'],
    })
  );
  // Now symlink an entire subdir from within `.deep-work` to that location.
  fs.mkdirSync(path.join(root, '.deep-work'), { recursive: true });
  fs.symlinkSync(
    path.join(outsideDir, 'fake-session'),
    path.join(root, '.deep-work/2026-05-11-evil-subdir')
  );
  ...
  // Out-of-boundary symlinked subdir rejected → no envelopes collected.
  assert.equal(handoffs.envelopes.length, 0);
  assert.ok(
    handoffs.failures.some((f) => f.reason === 'out-of-boundary-symlink'),
    `expected out-of-boundary-symlink failure, got: ${JSON.stringify(handoffs.failures)}`
...
test('M5 C2: in-tree symlink at per-session handoff path is allowed', SYMLINK_TEST_OPTIONS, async () => {
