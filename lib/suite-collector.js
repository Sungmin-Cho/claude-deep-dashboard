...
/**
 * Compare the payload schema MAJOR (env.schema.version, 'MAJOR.MINOR' per
 * the envelope schema) against the supported MAJOR whitelist
 * (PAYLOAD_SCHEMA_MAJOR). Returns a failure string on mismatch, null when
 * accepted. Missing/non-numeric versions are defensively treated as
 * mismatches; kinds without a whitelist entry are not gated.
 */
function payloadSchemaMajorMismatch(env, expectedProducer, expectedKind) {
  const expectedMajor = PAYLOAD_SCHEMA_MAJOR[`${expectedProducer}/${expectedKind}`];
  if (expectedMajor === undefined) return null;
  const got = env.schema.version;
  const major = Number(String(got).split('.')[0]);
  if (!Number.isInteger(major) || major !== expectedMajor) {
    return `schema-major-mismatch:${got}`;
  }
  return null;
}
 ...
 *   3. payload-schema-major → failure='schema-major-mismatch:<got>'
 *   4. payload-object-shape → failure='payload-shape-violation'
 *   5. payload-required-fields → failure='missing-required-fields:<csv>'
 */
function unwrapStrict(obj, expectedProducer, expectedKind, sourceLabel) {
  ...
  // Payload schema MAJOR guard (suite contract: MAJOR mismatch → skip +
  // warning; MINOR is additive and accepted). Without this, a breaking
  // MAJOR bump keeps the same schema.name, passes identity + v1.0
  // required-field checks, and aggregators silently compute wrong values.
  const majorFailure = payloadSchemaMajorMismatch(env, expectedProducer, expectedKind);
  if (majorFailure !== null) {
    return { failure: majorFailure, source: sourceLabel };
  }
  ...
  // deep-suite/schemas/payload-registry/<producer>/<kind>/v1.0.schema.json
  // `required` arrays).
...
const SOURCE_SPECS = [
  ...
  //     deep-suite/guides/long-run-handoff.md §4.1 and
 ...
 * deep-suite/guides/long-run-handoff.md §4.1; names colliding with a
...
function readSessionGlob(projectRoot, parentRel, filename, excludeDirName) {
  ...
  for (const ent of dirents) {
    ...
    // lstat observes the candidate directory entry without following a
    // symlink. This is deliberately separate from stat (target existence)
    // and realpath (canonical containment): Windows may report a dangling
    // symlink at this stage even when an access-style existence probe appears
    // to succeed.
    let candidateLstat;
    try {
      ...
      failures.push({ path: candidate, reason: 'candidate-lstat-failed' });
      continue;
    }

    const candidateIsSymlink = candidateLstat.isSymbolicLink();
    if (candidateIsSymlink) {
      try {
        // stat follows the link. ENOENT here uniquely identifies a dangling
        // target and is intentionally equivalent to a not-yet-emitted file.
        fs.statSync(candidate);
      } catch (err) {
        if (err && err.code === 'ENOENT') continue;
        failures.push({ path: candidate, reason: 'symlink-target-stat-failed' });
        continue;
      }
    }
    ...
    try {
      ...
      // A symlink target can disappear between stat and realpath. Preserve
      // dangling-link semantics for that race, but fail closed for every
      // other realpath error (and for a disappearing regular file).
      if (candidateIsSymlink && err && err.code === 'ENOENT') continue;
      failures.push({ path: candidate, reason: 'candidate-realpath-failed' });
      continue;
    }
...
function collectEnvelopeSource(spec, projectRoot) {
  ...
  if (spec.cardinality === 'dir+session-glob') {
    ...
    // Round 3 review fix (W4, Codex review P2): dedup by envelope.run_id
    // within the merged result. A producer that writes the same envelope to
    // BOTH flat and per-session layouts (transitional period, accidental
    // double-write) would otherwise inflate metrics that count envelopes
    // (compaction.frequency) or build chain indices (roundtrip denominator).
    // Flat-dir entries take precedence over session-glob entries on collision
    // (they appear first in `allEntries`); the second instance is logged as
    // a `duplicate-run-id` failure so the producer can detect the issue.
    const seenRunIds = new Set();
    for (const { path: p, parsed } of allEntries) {
      const r = unwrapStrict(parsed, spec.producer, spec.kind, p);
      if (r.failure) {
        out.failures.push({ reason: r.failure, source: r.source });
        continue;
      }
      const rid = r.envelope?.run_id;
      if (typeof rid === 'string' && rid.length > 0) {
        if (seenRunIds.has(rid)) {
          out.failures.push({ reason: 'duplicate-run-id', source: p });
          continue;
        }
        seenRunIds.add(rid);
      }
      out.envelopes.push(r);
    }
    return out;
  }
  return out;
 ...
 *   3. <projectRoot>/.deep-wiki/.wiki-meta/index.json  (project-local fallback)
 *
 * Round 1 review (Codex P2): previous implementation read `<wiki_root>/index.json`
 * which never exists in a real deep-wiki vault. The project-local fallback
 * had the same class of bug (missed the `.wiki-meta/` segment) — fixed to
 * match the explicit/env branches. (`resolveWikiLogPath`'s local fallback
 * needs no segment: log.jsonl lives at the vault root.)
 */
function resolveWikiIndexPath(projectRoot, options = {}) {
  ...
  return path.join(projectRoot, '.deep-wiki', '.wiki-meta', 'index.json');
}
