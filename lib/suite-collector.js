...
const SOURCE_SPECS = [
  ...
  // M5: deep-work emits handoff + compaction-state envelopes into per-kind dirs.
  // The dashboard uses a flat `.deep-work/handoffs/` and
  // `.deep-work/compaction-states/` layout (mirroring the existing
  // `.deep-work/receipts/` flat-dir pattern for slice receipts) so cross-
  // session aggregation works without a per-session-subdir walk. The
  // claude-deep-suite long-run-handoff guide describes the conceptual emit
  // location as `.deep-work/<session>/handoff.json`; producers MAY also (or
  // instead) drop a copy under `.deep-work/handoffs/<session>-<run_id>.json`
  // for dashboard consumption — that is the path the collector scans.
  {
    producer: 'deep-work',
    kind: 'handoff',
