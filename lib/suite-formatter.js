...
function renderValue(value, unit) {
  ...
  if (typeof value === 'object') {
    // Distribution — compact inline. Two leaf shapes are supported:
    //   - scalar (verdict_mix style):    { APPROVE: 3, CONCERN: 1 }
    //         → `APPROVE=3, CONCERN=1`
    //   - object cell carrying a `ratio` field (coverage_per_plugin style):
    //     { "deep-work": { ratio: 1.0, covered: 4, expected: 4, tests: [...] }, ... }
    //         → `deep-work=100%, deep-evolve=100%, ...`
    // The object-cell renderer surfaces the headline ratio for at-a-glance
    // comparison; the full per-plugin detail stays available via
    // source_summary + the JSONL time series record (suite-metrics.jsonl).
    const entries = Object.entries(value).map(([k, v]) => {
      if (v && typeof v === 'object' && !Array.isArray(v) && typeof v.ratio === 'number') {
        return `${k}=${(v.ratio * 100).toFixed(0)}%`;
      }
      return `${k}=${v}`;
    });
    return `{ ${entries.join(', ')} }`;
...
export function formatSuiteReportMarkdown(snapshot, previous = null) {
  ...
  // M4-core section — section count derived from the snapshot.
  // Post-M5.5 (2026-05-12): all 16 metrics are M4-core; counts may grow as
  // future milestones register new deferred metrics. Always render from the
  ...
  // M4-deferred section — omitted entirely when no deferred metrics remain
  // (post-M5.5 default). Reappears automatically when a future milestone
  // registers new deferred metrics; the header + table format are preserved
  // for that forward-compat path.
  if (deferredCount > 0) {
    lines.push(`## M4-deferred metrics (${deferredCount})`);
    lines.push('');
    lines.push(`Sources land in a future milestone; currently emit \`null\`.`);
    lines.push('');
