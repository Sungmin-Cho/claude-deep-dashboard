...
test('formatSuiteReportMarkdown renders all 17 metrics + collector self-metrics', async () => {
  ...
  // M4-deferred section is empty and intentionally omitted. 17 = 16 + the
  // honest replacement (ingest_actions_total) for the deprecated
  // auto_ingest_candidates_total wire key.
  assert.ok(md.includes('## M4-core metrics (17)'));
...
test('writeSuiteReportFile creates .deep-dashboard/suite-report.md', async () => {
  ...
  assert.ok(out.endsWith(path.join('.deep-dashboard', 'suite-report.md')));
