 ...
 * 2. Per-plugin adoption-since dates (mirrors deep-suite/docs/envelope-migration.md §6.1).
...
// ---------------------------------------------------------------------------
// Per-plugin adoption ledger (mirrors deep-suite docs/envelope-migration.md §6.1)
// ---------------------------------------------------------------------------
...
export const EXPECTED_SOURCES = Object.freeze([
  ...
  // M5 schemas ratified in deep-suite (PRs #12/#13 merged 2026-05-11) —
  ...
  // deep-suite/guides/long-run-handoff.md include the reverse handoff
...
export const PAYLOAD_REQUIRED_FIELDS = Object.freeze({
  ...
  // M5: cross-plugin handoff payload (deep-suite/schemas/handoff.schema.json,
  ...
  // M5: compaction-state payload (deep-suite/schemas/compaction-state.schema.json,
  // schema_version 1.0). Mirrors the schema's `required[]` keyword 1:1.
...
// ---------------------------------------------------------------------------
// PAYLOAD_SCHEMA_MAJOR — supported payload schema.version MAJOR per kind
// ---------------------------------------------------------------------------
// Suite contract (deep-suite schemas/README.md + envelope-migration.md):
// `envelope.schema.version` is the payload's MAJOR.MINOR, independent of the
// top-level envelope `schema_version`. Additive changes are MINOR bumps
// (1.0 → 1.1, must be accepted); breaking changes are MAJOR bumps that the
// dashboard's per-kind field assumptions (PAYLOAD_REQUIRED_FIELDS, aggregator
// field reads) no longer cover — unwrapping them would let metrics silently
// compute wrong values. The unwrap seams compare MAJORs against this
// whitelist and skip-with-warning on mismatch, mirroring suite
// validate-artifact.js's registry-lookup-by-version behaviour without
// shipping the registry itself.
//
// Single source: bump an entry here (one place) when a producer's MAJOR is
// approved for consumption.
export const PAYLOAD_SCHEMA_MAJOR = Object.freeze({
  'deep-work/session-receipt':            1,
  'deep-work/slice-receipt':              1,
  'deep-review/recurring-findings':       1,
  'deep-docs/last-scan':                  1,
  'deep-evolve/evolve-receipt':           1,
  'deep-evolve/evolve-insights':          1,
  'deep-dashboard/harnessability-report': 1,
  'deep-wiki/index':                      1,
  'deep-work/handoff':                    1,
  'deep-evolve/handoff':                  1,
  'deep-work/compaction-state':           1,
  'deep-evolve/compaction-state':         1,
});
