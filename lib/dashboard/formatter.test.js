...
// ---------------------------------------------------------------------------
// Regression: pad() must tolerate undefined/null input coming from any caller.
// Exercised end-to-end via renderFitness, whose pad(f.ruleId, 18) call site
// does not itself coalesce. Pre-hardening this would throw inside stripAnsi's
// .replace. Post-hardening it should produce padded empty space and keep going.
// ---------------------------------------------------------------------------

test('formatCLI does not throw when a fitness entry lacks ruleId (pad() self-defense)', () => {
  const data = {
    topology: 'unknown',
    harnessability: { total: 0, grade: 'Poor' },
    effectiveness: null,
    health: [],
    fitness: [
      { passed: true, violations: [] }, // ruleId intentionally omitted
    ],
    sessions: [],
    actions: [],
  };
