 ...
 *   For artifacts that producers emit in the deep-suite cross-plugin
 *   envelope (cf. deep-suite/docs/envelope-migration.md), this collector
...
import { PAYLOAD_SCHEMA_MAJOR } from '../suite-constants.js';
...
function unwrapEnvelope(obj, expectedProducer, expectedKind, sourceLabel) {
  ...
  // Payload schema MAJOR guard (mirror of suite-collector unwrapStrict —
  // duplication accepted until the M5 envelope-unwrap consolidation): a
  // breaking MAJOR bump keeps schema.name, so identity alone would trust a
  // payload whose v1.x field assumptions no longer hold. MINOR bumps are
  // additive and accepted; missing/non-numeric versions defensively reject.
  const expectedMajor = PAYLOAD_SCHEMA_MAJOR[`${expectedProducer}/${expectedKind}`];
  if (expectedMajor !== undefined) {
    const got = env.schema.version;
    const major = Number(String(got).split('.')[0]);
    if (!Number.isInteger(major) || major !== expectedMajor) {
      console.warn(
        `[deep-dashboard/collector] rejecting envelope at ${sourceLabel}: ` +
          `payload schema MAJOR mismatch (supported MAJOR=${expectedMajor}, ` +
          `got schema.version=${JSON.stringify(got)})`
      );
      return null;
    }
  }
...
/**
 * Collect the union of session receipts from both emit locations, newest
 * first, capped at the 3 most recent.
 *
 * deep-work emits session receipts at the legacy root path
 * `.deep-work/session-receipt.json` AND at the real per-session path
 * `.deep-work/<sid>/session-receipt.json` — the root path alone misses
 * nested-only projects, so both are unioned (compat: root-only projects
 * must keep working).
 *
 * Ordering: payload `started_at` (schema-required, deterministic) is the
 * recency key for every receipt, root included; file mtime is the fallback
 * only when `started_at` is absent or unparseable (legacy artifacts).
 * De-dup runs BEFORE the top-3 cut — the same session present at both
 * root and nested paths must be counted once (envelope `run_id` first,
 * payload `session_id` fallback).
 */
function collectSessionReceipts(deepWorkDir, rootReceiptPath) {
  const candidatePaths = [];

  if (pathExists(rootReceiptPath)) candidatePaths.push(rootReceiptPath);

  let entries = [];
  try {
    entries = fs.readdirSync(deepWorkDir, { withFileTypes: true });
  } catch {
    entries = [];
  }

  // Containment guard (mirrors readJsonDir, collector.js:150-182): `entry.isDirectory()`
  // above already rejects a symlinked <sid> directory, but the session-receipt.json
  // FILE inside a real <sid> directory can itself be a symlink pointing outside
  // .deep-work. Canonicalize the scan root once and reject any nested candidate
  // whose resolved target escapes it, same as readJsonDir's out-of-boundary check.
  let resolvedDeepWorkDir;
  try {
    resolvedDeepWorkDir = fs.realpathSync(deepWorkDir);
  } catch {
    resolvedDeepWorkDir = null;
  }

  for (const entry of entries) {
    if (!entry.isDirectory()) continue;
    const nested = path.join(deepWorkDir, entry.name, 'session-receipt.json');
    if (!pathExists(nested)) continue;

    let lstat;
    try {
      lstat = fs.lstatSync(nested);
    } catch {
      continue;
    }

    if (lstat.isSymbolicLink()) {
      if (resolvedDeepWorkDir === null) {
        console.warn(`[deep-dashboard/collector] skipping out-of-boundary symlink: ${nested} (scan root unresolvable)`);
        continue;
      }
      let resolvedTarget;
      try {
        resolvedTarget = fs.realpathSync(nested);
      } catch {
        console.warn(`[deep-dashboard/collector] skipping broken symlink: ${nested}`);
        continue;
      }
      const rel = path.relative(resolvedDeepWorkDir, resolvedTarget);
      const outOfBoundary = rel === '' || rel.startsWith('..') || path.isAbsolute(rel);
      if (outOfBoundary) {
        console.warn(`[deep-dashboard/collector] skipping out-of-boundary symlink: ${nested} -> ${resolvedTarget}`);
        continue;
      }
    }
  ...
  for (const absPath of candidatePaths) {
    ...
    const payload = unwrapEnvelope(raw, 'deep-work', 'session-receipt', absPath);
    if (payload === null) continue;

    let sortKey = Date.parse(payload.started_at);
    if (!Number.isFinite(sortKey)) {
      try {
        sortKey = fs.statSync(absPath).mtimeMs;
      } catch {
        sortKey = 0;
      }
    }

    const envRunId = isEnvelopeShape(raw) ? raw.envelope.run_id : undefined;
    const dedupKey =
      (typeof envRunId === 'string' && envRunId !== '' ? envRunId : null) ??
      (typeof payload.session_id === 'string' && payload.session_id !== '' ? payload.session_id : null);

    collected.push({ payload, sortKey, dedupKey });
  }
  ...
  // README contract: average quality_score of the last 3 sessions ("last 3"
  // is an upper bound — fewer receipts average what exists).
  return deduped.slice(0, 3).map((r) => r.payload);
}
 ...
 *   <root>/.deep-work/<sid>/session-receipt.json — per-session receipts (real emit path)
 *
...
function collectDeepWork(root) {
  const deepWorkDir = path.join(root, '.deep-work');
  const receiptsDir = path.join(deepWorkDir, 'receipts');
  const sessionReceiptPath = path.join(deepWorkDir, 'session-receipt.json');

  const receipts = readJsonDir(receiptsDir)
    .map((obj) => unwrapEnvelope(obj, 'deep-work', 'slice-receipt', `${receiptsDir}/<entry>`))
    .filter((v) => v !== null);

  // session-receipt single-file field kept for downstream compat
  ...
  const sessionReceipts = collectSessionReceipts(deepWorkDir, sessionReceiptPath);

  const hasAny = receipts.length > 0 || sessionReceipt !== null || sessionReceipts.length > 0;
 ...
 * @returns {{
 *   deepWork: { status: string, receipts: object[], sessionReceipt: object|null, sessionReceipts: object[] },
