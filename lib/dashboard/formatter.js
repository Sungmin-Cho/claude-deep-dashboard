...
function renderEvolveCLI(evolve) {
  ...
  const crashPct = Math.round(((exp.crashed ?? 0) / (exp.total || 1)) * 100);
  ...
  const qualStr = `${r.quality_score}/100${r.outcome === 'discarded' ? ' (discarded)' : ''}`;

  const lines = [];
  lines.push(divider());
  lines.push(centerLine('Evolve'));
  lines.push(boxLine(`  Experiments: ${exp.total} (keep: ${keepPct}%, crash: ${crashPct}%)`));
  lines.push(boxLine(`  Improvement: +${improv}%`));
  lines.push(boxLine(`  Strategy: ${gens} gen, Q: ${qFirst} → ${qLast}`));
  lines.push(boxLine(`  Archives: ${arch.strategy_archive_size} strategies, ${arch.code_archive_size} code, ${arch.code_forks_used} forks`));
...
function renderEvolveMarkdown(evolve) {
  ...
  parts.push('|--------|-------|');
  parts.push(`| Experiments | ${exp.total} (keep: ${keepPct}%, crash: ${crashPct}%) |`);
  parts.push(`| Improvement | +${improv}% from baseline |`);
  parts.push(`| Strategy Evolution | ${gens} generations, Q: ${qFirst} → ${qLast} |`);
  parts.push(`| Archives | ${arch.strategy_archive_size} strategies, ${arch.code_archive_size} code snapshots, ${arch.code_forks_used} forks |`);
