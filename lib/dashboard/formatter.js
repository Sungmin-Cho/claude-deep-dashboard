...
function pad(str, width) {
  ...
  // Self-defensive: undefined/null/number inputs are coerced to a string
  // so a new call site can't accidentally trigger a .replace crash downstream.
  const safe = String(str ?? '');
  const visible = stripAnsi(safe);
  const extra = width - visible.length;
  return safe + ' '.repeat(Math.max(0, extra));
}

function stripAnsi(str) {
  // Remove ANSI escape codes for width calculation.
  // Coerces to string so pad() and any other caller can't pass undefined through.
