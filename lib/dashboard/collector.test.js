...
import { probeSymlinkCapability } from '../test-support/symlink-capability.js';
...
/**
 * Run `fn` with `console.warn` suppressed; restore the original after.
 * Used by tests that intentionally trigger collector warnings (identity-guard
 * rejection, corrupt-payload rejection) to keep `npm test` output clean.
 */
function withSuppressedWarn(fn) {
...
test('rejects envelope with mismatched producer (defense-in-depth identity guard)', () => {
  ...
  const result = withSuppressedWarn(() => collectData(root));
...
// ---------------------------------------------------------------------------
// sessionReceipts union (root + nested <sid>/session-receipt.json)
// ---------------------------------------------------------------------------
//
// deep-work emits session receipts at BOTH the legacy root path
// `.deep-work/session-receipt.json` AND the real per-session path
// `.deep-work/<sid>/session-receipt.json`. The collector must union both,
// order by payload `started_at` (newest first, file mtime only as fallback),
// de-dup identical sessions, and keep at most the 3 most recent.

function sessionReceiptPayload(sessionId, startedAt, qualityScore) {
  return {
    schema_version: '1.0',
...
test('sessionReceipts: root-only receipt still collected (compat regression)', () => {
  const root = mktemp();
  const wrapped = envelopeWrap(
    'deep-work',
    'session-receipt',
    sessionReceiptPayload('s-root', '2026-05-01T10:00:00Z', 88),
  ...
  assert.equal(result.deepWork.status, 'available');
  assert.equal(result.deepWork.sessionReceipts.length, 1);
  assert.equal(result.deepWork.sessionReceipts[0].quality_score, 88);
});

test('sessionReceipts: nested-only receipts collected and status available', () => {
  const root = mktemp();
  writeJson(
    root,
    '.deep-work/20260501-100000-task/session-receipt.json',
    envelopeWrap('deep-work', 'session-receipt', sessionReceiptPayload('s1', '2026-05-01T10:00:00Z', 70), { runId: '01AAAAAAAAAAAAAAAAAAAAAAA1' })
  );
  ...
  assert.equal(result.deepWork.status, 'available');
  assert.equal(result.deepWork.sessionReceipts.length, 1);
  assert.equal(result.deepWork.sessionReceipts[0].quality_score, 70);
});

test('sessionReceipts: keeps 3 most recent by started_at, not by sid name', () => {
  // SID directory names deliberately INVERTED w.r.t. started_at so a
  // sid-lex-sort implementation cannot pass: the lexicographically LAST
  // sid holds the OLDEST started_at.
  const root = mktemp();
  const entries = [
    // [sidDir, started_at, quality_score, runId]
    ['sid-d', '2026-05-01T10:00:00Z', 10, '01AAAAAAAAAAAAAAAAAAAAAAA1'], // oldest, lex-last... see below
    ['sid-c', '2026-05-02T10:00:00Z', 20, '01AAAAAAAAAAAAAAAAAAAAAAA2'],
    ['sid-b', '2026-05-03T10:00:00Z', 30, '01AAAAAAAAAAAAAAAAAAAAAAA3'],
    ['sid-a', '2026-05-04T10:00:00Z', 40, '01AAAAAAAAAAAAAAAAAAAAAAA4'], // newest, lex-FIRST
  ];
  for (const [sid, startedAt, qs, runId] of entries) {
    writeJson(
      root,
      `.deep-work/${sid}/session-receipt.json`,
      envelopeWrap('deep-work', 'session-receipt', sessionReceiptPayload(`s-${sid}`, startedAt, qs), { runId })
    );
  }

  const result = collectData(root);

  // Newest 3 by started_at = qs 40, 30, 20. sid lex-sort ascending would
  // instead pick [sid-a, sid-b, sid-c] as FIRST names but a "latest = last"
  // lex heuristic would pick [sid-d, sid-c, sid-b] (qs 10, 20, 30) — both
  // orderings differ from the started_at truth in at least one member.
  const scores = result.deepWork.sessionReceipts.map((r) => r.quality_score);
  assert.deepEqual(scores, [40, 30, 20]);
});

test('sessionReceipts: root receipt excluded when it is 4th-oldest in the union', () => {
  const root = mktemp();
  writeJson(
    root,
    '.deep-work/session-receipt.json',
    envelopeWrap('deep-work', 'session-receipt', sessionReceiptPayload('s-root', '2026-04-01T10:00:00Z', 5), { runId: '01AAAAAAAAAAAAAAAAAAAAAAA0' })
  );
  const nested = [
    ['sid-1', '2026-05-01T10:00:00Z', 10, '01AAAAAAAAAAAAAAAAAAAAAAA1'],
    ['sid-2', '2026-05-02T10:00:00Z', 20, '01AAAAAAAAAAAAAAAAAAAAAAA2'],
    ['sid-3', '2026-05-03T10:00:00Z', 30, '01AAAAAAAAAAAAAAAAAAAAAAA3'],
  ];
  for (const [sid, startedAt, qs, runId] of nested) {
    writeJson(
      root,
      `.deep-work/${sid}/session-receipt.json`,
      envelopeWrap('deep-work', 'session-receipt', sessionReceiptPayload(`s-${sid}`, startedAt, qs), { runId })
  ...
  const scores = result.deepWork.sessionReceipts.map((r) => r.quality_score);
  assert.deepEqual(scores, [30, 20, 10], 'root receipt (oldest) must fall out of the top-3 window');
});

test('sessionReceipts: same session at root and nested counted once (envelope run_id de-dup)', () => {
  const root = mktemp();
  const sharedRunId = '01DUPDUPDUPDUPDUPDUPDUPDUP';
  const payload = sessionReceiptPayload('s-dup', '2026-05-03T10:00:00Z', 90);
  writeJson(root, '.deep-work/session-receipt.json', envelopeWrap('deep-work', 'session-receipt', payload, { runId: sharedRunId }));
  writeJson(root, '.deep-work/sid-dup/session-receipt.json', envelopeWrap('deep-work', 'session-receipt', payload, { runId: sharedRunId }));
  writeJson(
    root,
    '.deep-work/sid-other/session-receipt.json',
    envelopeWrap('deep-work', 'session-receipt', sessionReceiptPayload('s-other', '2026-05-01T10:00:00Z', 30), { runId: '01AAAAAAAAAAAAAAAAAAAAAAA1' })
  );

  const result = collectData(root);

  // Double-counting the duplicate would yield [90, 90, 30] and skew the average.
  const scores = result.deepWork.sessionReceipts.map((r) => r.quality_score);
  assert.deepEqual(scores, [90, 30]);
});

test('sessionReceipts: legacy receipts de-dup by payload session_id fallback', () => {
  // Legacy (un-enveloped) receipts have no envelope run_id — the payload
  // session_id is the fallback identity.
  const root = mktemp();
  const legacy = sessionReceiptPayload('s-legacy', '2026-05-03T10:00:00Z', 80);
  writeJson(root, '.deep-work/session-receipt.json', legacy);
  writeJson(root, '.deep-work/sid-legacy/session-receipt.json', legacy);

  const result = collectData(root);

  const scores = result.deepWork.sessionReceipts.map((r) => r.quality_score);
  assert.deepEqual(scores, [80]);
});

test('sessionReceipts: identity-mismatched envelope receipt excluded from union', () => {
  const root = mktemp();
  writeJson(
    root,
    '.deep-work/sid-good/session-receipt.json',
    envelopeWrap('deep-work', 'session-receipt', sessionReceiptPayload('s-good', '2026-05-01T10:00:00Z', 60), { runId: '01AAAAAAAAAAAAAAAAAAAAAAA1' })
  );
  writeJson(
    root,
    '.deep-work/sid-evil/session-receipt.json',
    envelopeWrap('deep-evolve', 'evolve-receipt', sessionReceiptPayload('s-evil', '2026-05-02T10:00:00Z', 99), { runId: '01AAAAAAAAAAAAAAAAAAAAAAA2' })
  );

  const result = withSuppressedWarn(() => collectData(root));

  const scores = result.deepWork.sessionReceipts.map((r) => r.quality_score);
  assert.deepEqual(scores, [60]);
});

test('sessionReceipts: mtime fallback orders receipts lacking parseable started_at', () => {
  const root = mktemp();
  // Two legacy receipts without started_at — recency must come from mtime.
  writeJson(root, '.deep-work/sid-old/session-receipt.json', { session_id: 's-old', quality_score: 10 });
  writeJson(root, '.deep-work/sid-new/session-receipt.json', { session_id: 's-new', quality_score: 20 });
  const past = new Date('2026-01-01T00:00:00Z');
  const recent = new Date('2026-06-01T00:00:00Z');
  fs.utimesSync(path.join(root, '.deep-work/sid-old/session-receipt.json'), past, past);
  fs.utimesSync(path.join(root, '.deep-work/sid-new/session-receipt.json'), recent, recent);

  const result = collectData(root);

  const scores = result.deepWork.sessionReceipts.map((r) => r.quality_score);
  assert.deepEqual(scores, [20, 10]);
});
...
test('rejects envelope with payload schema MAJOR mismatch (v2.0) → null + warn', () => {
  // Mirror of the suite-collector guard: a breaking MAJOR payload must not be
  // trusted under v1.x field assumptions (scorer would read renamed fields).
  const root = mktemp();
  const wrapped = envelopeWrap('deep-work', 'session-receipt', {
    session_id: 's1',
    started_at: '2026-05-01T10:00:00Z',
    quality_score: 88,
  });
  wrapped.envelope.schema.version = '2.0';
  writeJson(root, '.deep-work/session-receipt.json', wrapped);

  const result = withSuppressedWarn(() => collectData(root));

  assert.equal(result.deepWork.sessionReceipt, null);
  assert.deepEqual(result.deepWork.sessionReceipts, []);
});

test('accepts envelope with payload schema MINOR bump (v1.1, additive)', () => {
  const root = mktemp();
  const wrapped = envelopeWrap('deep-work', 'session-receipt', {
    session_id: 's1',
    started_at: '2026-05-01T10:00:00Z',
  ...
  wrapped.envelope.schema.version = '1.1';
  writeJson(root, '.deep-work/session-receipt.json', wrapped);
...
test('rejects envelope with non-object payload (array)', () => {
  // Round-5/7 corrupt-payload defense extension: even with valid envelope and
  // matching identity, a non-object payload is hostile/buggy and must resolve
  // to null at the unwrap seam so downstream consumers never see undefined-keyed
  // primitives. Tested with array (the common JS gotcha — typeof [] === 'object').
  const root = mktemp();
  const wrapped = envelopeWrap('deep-dashboard', 'harnessability-report', [1, 2, 3]);
  writeJson(root, '.deep-dashboard/harnessability-report.json', wrapped);
...
test('rejects envelope with null payload (corrupt envelope)', () => {
  // Same defense: payload: null at unwrap → null result → status: no_data.
  const root = mktemp();
  const wrapped = envelopeWrap('deep-dashboard', 'harnessability-report', null);
  writeJson(root, '.deep-dashboard/harnessability-report.json', wrapped);
...
test('rejects envelope with primitive payload (corrupt envelope)', () => {
  const root = mktemp();
  const wrapped = envelopeWrap('deep-dashboard', 'harnessability-report', 7);
  writeJson(root, '.deep-dashboard/harnessability-report.json', wrapped);
...
const { skip: SYMLINK_SKIP_REASON } = probeSymlinkCapability();
...
// ---------------------------------------------------------------------------
// P2 regression: nested session-receipt.json symlink must respect .deep-work
// containment (mirrors readJsonDir's out-of-boundary guard, collector.js:176-182)
// ---------------------------------------------------------------------------

test(
  'collectSessionReceipts rejects a nested session-receipt.json symlink whose target resolves OUTSIDE .deep-work',
  { skip: SYMLINK_SKIP_REASON },
  () => {
    const root = mktemp();

    // In-boundary nested receipt — must still be collected (no regression).
    writeJson(
      root,
      '.deep-work/sid-good/session-receipt.json',
      envelopeWrap('deep-work', 'session-receipt', sessionReceiptPayload('s-good', '2026-05-01T10:00:00Z', 60), { runId: '01AAAAAAAAAAAAAAAAAAAAAAA1' })
    ...
    // Out-of-tree target: a session-receipt.json living entirely outside .deep-work.
    const outsideDir = fs.mkdtempSync(path.join(os.tmpdir(), 'collector-outside-receipt-'));
    const outsideTarget = path.join(outsideDir, 'session-receipt.json');
    fs.writeFileSync(
      outsideTarget,
      JSON.stringify(
        envelopeWrap('deep-work', 'session-receipt', sessionReceiptPayload('s-evil', '2026-05-02T10:00:00Z', 99), { runId: '01AAAAAAAAAAAAAAAAAAAAAAA2' })
      )
    );

    // The <sid> directory itself is real; only the session-receipt.json FILE inside it is a symlink.
    const evilSidDir = path.join(root, '.deep-work', 'sid-evil');
    fs.mkdirSync(evilSidDir, { recursive: true });
    fs.symlinkSync(outsideTarget, path.join(evilSidDir, 'session-receipt.json'));

    const result = withSuppressedWarn(() => collectData(root));

    const scores = result.deepWork.sessionReceipts.map((r) => r.quality_score);
    assert.ok(!scores.includes(99), 'out-of-tree symlinked session-receipt.json must NOT be ingested');
    assert.deepEqual(scores, [60], 'in-boundary nested receipt still collected; out-of-tree symlink excluded');
  }
);
