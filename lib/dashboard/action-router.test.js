...
test("docs-stale detail reports accurate file count when > 3 affected files", () => {
  // Round-4 Opus Info polish: previously sampleFiles.length (saturated at 3)
  // was displayed regardless of actual affected file count. With 5 affected
  // files the detail should now read "5 file(s)" not "3+ file(s)".
  const data = makeDocsData({
    documents: [
      { path: "a.md", issues: [{ severity: "medium" }] },
      { path: "b.md", issues: [{ severity: "medium" }] },
      { path: "c.md", issues: [{ severity: "medium" }] },
      { path: "d.md", issues: [{ severity: "medium" }] },
      { path: "e.md", issues: [{ severity: "medium" }] },
    ],
  });
  const actions = getSuggestedActions(data);
  const docsAction = actions.find((a) => a.finding === "docs-stale");
  assert.ok(docsAction);
  assert.match(docsAction.detail, /5 doc issue/);
  assert.match(docsAction.detail, /5 file/);
  // Sample files cap at 3 with truncation suffix
  assert.match(docsAction.detail, /a\.md, b\.md, c\.md\.\.\./);
});
