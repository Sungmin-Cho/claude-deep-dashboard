 ...
 *
 * Calculates an overall harness effectiveness score (0–10) from five
...
/**
 * Extract a 0–10 session score from deep-work session receipts.
 *
 * quality_score is a session-level aggregate: it exists only on
 * session-receipts (payload-registry deep-work/session-receipt), never on
 * slice receipts (additionalProperties: false). The collector supplies
 * `sessionReceipts` — the union of root and nested session-receipt
 * payloads, newest first, capped at 3.
 *
 * Averages the quality_score fields (0–100), then normalizes to 0–10.
 */
function extractSessionScore(data) {
  const receipts = data.deepWork?.sessionReceipts;
  if (!Array.isArray(receipts) || receipts.length === 0) return null;

  // Spec §5.5: use last 3 sessions only (collector caps at 3; the slice
  // here is defense against longer hand-built lists — list is newest-first)
  const recent = receipts.slice(0, 3);
  const scores = recent
    .map((r) => (r !== null && typeof r === 'object' ? r.quality_score ?? null : null))
