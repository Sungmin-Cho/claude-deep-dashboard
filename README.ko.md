...
- `/deep-harness-dashboard` (4개 효과성 입력 중 하나로)
...
4개의 가중 차원으로 0–10점의 단일 효과성 점수를 계산합니다:

| 차원 | 가중치 | 소스 |
|---|---|---|
| Health | 30% | deep-review fitness 데이터의 `sensors_clean_ratio` |
| Fitness | 25% | `.deep-review/fitness.json`의 `rules_pass_ratio` |
| Session | 25% | 최근 3개 deep-work receipt의 평균 `quality_score` (0–100 정규화 → 0–10) |
| Harnessability | 20% | `.deep-dashboard/harnessability-report.json`의 `total` |
