...
test('every skill and always-loaded markdown file has balanced code fences', () => {
  ...
  // Parity is a proxy, and it detects neither real failure. A sibling shipped a
  // SKILL.md with fourteen markers — even, so this sweep passed — where a ```bash with
  // an info string failed to close the ```markdown template above it, a later bare
  // marker closed that template instead, and the final block was never closed. Half the
  // document rendered as code. The parity count stays (it catches a truncating split
  // cheaply); CommonMark's own rule is asserted beside it: a closer matches the
  // opener's character, is at least as long, and carries NO info string.
  ...
  assert.equal(openAtEof('````markdown\n```bash\nx\n```\nmore\n````\n'), false,
    'and a longer outer fence nests correctly, which is the fix');
